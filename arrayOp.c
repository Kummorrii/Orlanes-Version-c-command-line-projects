...
void printMainMenu()
{
    ...
    printf("\t╔═══════════════════════════════════════════════════════╗\n");
    printf("\t║" ANSI_BLUE "\t\t~~~Array Operations~~~\t\t" ANSI_RESET "\t║\n");
    printf("\t║" ANSI_BLUE "\t\tA.) Initialize Array\t\t" ANSI_RESET "\t║\n");
    printf("\t║" ANSI_BLUE "\t\tB.) Print Array\t\t\t" ANSI_RESET "\t║\n");
    printf("\t║" ANSI_BLUE "\t\tC.) Insert At Position\t\t" ANSI_RESET "\t║\n");
    printf("\t║" ANSI_BLUE "\t\tD.) Insert Front\t\t" ANSI_RESET "\t║\n");
    printf("\t║" ANSI_BLUE "\t\tE.) Remove At Position\t\t" ANSI_RESET "\t║\n");
    printf("\t║" ANSI_BLUE "\t\tF.) Remove Front\t\t" ANSI_RESET "\t║\n");
    printf("\t║" ANSI_BLUE "\t\tG.) Locate Data Item\t\t" ANSI_RESET "\t║\n");
    printf("\t║" ANSI_BLUE "\t\tH.) SortAscending(Bubble Sort)\t" ANSI_RESET "\t║\n");
    printf("\t║" ANSI_BLUE "\t\tI.) SortDescending(Bubble Sort)\t" ANSI_RESET "\t║\n");
    printf("\t║" ANSI_BLUE "\t\tJ.) Exit\t\t\t" ANSI_RESET "\t║\n");
    printf("\t║\t\t\t\t\t\t\t║\n");
...
{
    if (*array == NULL) { clearScreen(); printError(4); return; }
    if (*size + 1 > ARRAY_SIZE_LIMIT) { clearScreen(); printError(3); return; }

    *size += 1;
    int position, value;
    int *temp = realloc(*array, (*size) * sizeof(**array));
    if (temp == NULL) { printError(5); return; }
    *array = temp;

    clearScreen();
    printf("Position to insert (0-%d): ", *size - 1);
    if (scanf("%d", &position) != 1 || position < 0 || position > *size - 1)
    {
        printError(6);
        *size -= 1;
...
{
    clearScreen();
    if (*array == NULL) { printError(4); return; }
    if (*size + 1 > ARRAY_SIZE_LIMIT) { printError(3); return; }
...
void removeAtPosition(int **array, int *size)
{
    ...
    if (*size == 1)
    {
        ...
        *array = NULL;
        *size = 0;
        puts("Array deleted — no elements remain.");
        while ((getchar()) != '\n');
        getchar();
        return;
    }
    ...
    int position;
    printf("Position to remove (0-%d): ", *size - 1);
    if (scanf("%d", &position) != 1 || position < 0 || position > *size - 1)
    {
        printError(6);
        return;
